...
interface ImgWithButtonProps extends React.PropsWithChildren, React.ImgHTMLAttributes<HTMLImageElement> {
  alignBtn: 'left' | 'right';
  imgH?: string;
  isRelativeBg?: boolean;
}

const ImgWithButton = ({ src, className, children, alignBtn = 'right', imgH, isRelativeBg }: ImgWithButtonProps) => {
  return (
    <div className={cn('relative w-full overflow-hidden', isRelativeBg && imgH, className)}>
      <img
        className={cn('w-full bg-cover', imgH, isRelativeBg && 'absolute top-1/2 h-auto w-full -translate-y-1/2')}
        src={src}
        alt="collection img"
      />
...
const BannerBtnImg = () => {
  return (
    <div className="flex items-stretch justify-between">
      <div className="mr-6 flex basis-1/2 flex-col items-center justify-start">
        <ImgWithButton
          imgH="h-[518px]"
          className="mb-16"
