...
const getVariantClasses = (variant: Variants): TagProps => {
  switch (variant) {
    case 'h1':
      return {
        classes: 'text-40 md:text-80 font-heading tracking-paragraph font-medium',
        tag: 'h1',
      };
    case 'h2':
      return {
        classes: 'text-32 md:text-56 font-heading tracking-paragraph font-medium',
        tag: 'h2',
      };
    case 'h3':
      return {
...
const Text = <V extends Variants, K extends ElementType = VariantTypeMap[V]>({
  ...
  className,
  ...p
}: TextProps<K, V> & Omit<ComponentPropsWithoutRef<K>, keyof TextProps<K, V>>) => {
